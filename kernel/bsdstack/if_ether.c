...
#define MAX_ARP_QUEUE 3//LUOYU add for ip fragment in this logic ip stack
#define	rt_expire rt_rmx.rmx_expire

struct llinfo_arp {
	LIST_ENTRY(llinfo_arp) la_le;
	struct	rtentry *la_rt;
	struct	mbuf *la_hold[3];	/* last packet until resolved/timeout */
...
static struct{								\
	struct llinfo_arp *lh_first;	/* first element */			\
}llinfo_arp;
	...
	struct mbuf *m;
{
	...
	int op;
	int req_len;
	int idx = 0; //LUOYU add for arp queue
...
// 		}
		...
		for (idx  = 0; idx < MAX_ARP_QUEUE; idx ++)
		{
			if (la->la_hold[idx]) {
				(*ifp->if_output)(ifp, la->la_hold[idx], rt_key(rt), rt);
				la->la_hold[idx] = 0;
			}
		}
	}
	...
	struct rt_addrinfo *info;
{
	...
	TAILQ_FOREACH(ifa, &rt->rt_ifp->if_addrhead, ifa_link) {  /*KJZ*/
	...
	struct sockaddr *dst, u_char *desten)
{
   ...
   int idx = 0;//added by LUOYU
	...
	for (idx = 0; idx < MAX_ARP_QUEUE; idx ++)
	{
	   if (la->la_hold[idx] == NULL)
	   {
	      la->la_hold[idx] = m;
	      break;
	   }
   	if (la->la_hold[MAX_ARP_QUEUE - 1] != NULL)// just replace the last one
   	{	
   	   m_freem(la->la_hold);
   	   la->la_hold[MAX_ARP_QUEUE - 1] = m;
   	   break;
   	}
	}
	if (rt->rt_expire) {
