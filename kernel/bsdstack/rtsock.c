...
#define	RTSOCK_LOCK()
#define	RTSOCK_UNLOCK()
#define	RTSOCK_LOCK_ASSERT()
...
rts_input(struct mbuf *m)
{
    ...
    bsd_raw_input(m, &route_proto, &route_src, &route_dst);
}
...
#define senderr(e) { error = e; goto flush;}
    ...
    //rtm->rtm_pid = curproc->p_pid;LUOYU
     ...
     */
    //if (rtm->rtm_type != RTM_GET)// && (error = cap_check_td(NULL, curthread, CAP_NET_ADMIN, 0)) != 0)
    //	senderr(error);LUOYU del for add route
...
#endif /* SCTP */	if (route_cb.any_count == 0)
    return;
