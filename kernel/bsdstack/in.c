	...
	struct thread *td)
{
	...
	case SIOCALIFADDR:
	case SIOCDLIFADDR:
		//if (td && (error = cap_check_td(NULL, td, CAP_NET_ADMIN, 0)) != 0)
		//	return error;
		/*fall through*/
	case SIOCGLIFADDR:
		if (!ifp)
	...
	if (ifp) {
        ...
        if( index( ifr->ifr_name , ':' ) ) {
		ia = NULL;
	        TAILQ_FOREACH(ifa, &ifp->if_addrhead, ifa_link) {
			struct in_ifaddr *tmpia = ifatoia(ifa);
		        if( !strcmp(tmpia->ia_name, ifr->ifr_name) ) {
			          ia = tmpia;
			          break;
			}
		}
	...
	case SIOCSIFDSTADDR:
		...
		if (ia == (struct in_ifaddr *)0) {
			...
			ia->ia_ifp = ifp;
			/*KJZ: add the new name */
                        strcpy(ia->ia_name, ifr->ifr_name);
			splx(s);
			iaIsNew = 1;
		}
		break;
	...
	case SIOCAIFADDR:
		...
		//if (error == 0)
		//	EVENTHANDLER_INVOKE(ifaddr_event, ifp); LUOYU
		return (error);
/*KJZ move this case down to here */
	case SIOCSIFADDR:
		...
		if (error == 0)
		    ...
		    return (0);
                ...
                /* I want to remove the 0.0.0.0 alias, just FALL THROUGH */
