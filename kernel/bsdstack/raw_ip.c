...
rip_init()
{
	 ...
	 */
	ripcbinfo.hashbase = hashinit(1, NULL, &ripcbinfo.hashmask);
	ripcbinfo.porthashbase = hashinit(1, NULL, &ripcbinfo.porthashmask);
	ripcbinfo.ipi_zone = uma_zcreate("ripcb", sizeof(struct inpcb), 100,
...
rip_ctloutput(struct socket *so, struct sockopt *sopt)
{
	...
	case SOPT_GET:
		...
		//	error = suser(curthread); // GFKD DY
			//error = cap_check_td(NULL, curthread, CAP_NET_RAW, 0);
			//if (error != 0)
			//	return (error);
...
extern struct	ifnethead ifnet_head;	/* depend on static init XXX */
