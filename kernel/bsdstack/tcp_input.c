 ...
 *///!callout_active(tp->tt_delack) &&	LUOYU
#define DELAY_ACK(tp)							\
	((			\
...
tcp_reass_init()
{
	...
	TUNABLE_INT_FETCH("net.inet.tcp.reass.maxsegments",
	    &tcp_reass_maxseg);
	tcp_reass_zone = uma_zcreate("tcpreass", sizeof (struct tseg_qent), 5,
	...
	struct mbuf *m;
{
	 ...
	 * just drop the pkt.
	 */
	te = uma_alloc(tcp_reass_zone, M_NOWAIT);
	if (te == NULL) {
		tcpstat.tcps_rcvmemdrop++;
		m_freem(m);
		return (0);
...
char *get_tcp_state(int state)
{
   static char stateStr[20] = {0};
   switch(state)
   {
   case TCPS_CLOSED:
      strcpy(stateStr, "TCPS_CLOSED");
      break;
   case TCPS_LISTEN:
      strcpy(stateStr, "TCPS_LISTEN");
      break;
   case TCPS_SYN_SENT:
      strcpy(stateStr, "TCPS_SYN_SENT");
      break;
   case TCPS_SYN_RECEIVED:
      strcpy(stateStr, "TCPS_SYN_RECEIVED");
      break;
   case TCPS_ESTABLISHED:
      strcpy(stateStr, "TCPS_ESTABLISHED");
      break;
   case TCPS_CLOSE_WAIT:
      strcpy(stateStr, "TCPS_CLOSE_WAIT");
      break;
   case TCPS_FIN_WAIT_1:
      strcpy(stateStr, "TCPS_FIN_WAIT_1");
      break;
   case TCPS_CLOSING:
      strcpy(stateStr, "TCPS_CLOSING");
      break;
   case TCPS_LAST_ACK:
      strcpy(stateStr, "TCPS_LAST_ACK");
      break;
   case TCPS_FIN_WAIT_2:
      strcpy(stateStr, "TCPS_FIN_WAIT_2");
      break;
   case TCPS_TIME_WAIT:
      strcpy(stateStr, "TCPS_TIME_WAIT");
      break;
   }
   return stateStr;
}

char *get_pkt_state(int state)
{
   static char pktstateStr[20] = {0};
   switch(state)
   {
   case TH_FIN:
      strcpy(pktstateStr, "TH_FIN");
      break;
   case TH_SYN:
      strcpy(pktstateStr, "TH_SYN");
      break;
   case TH_RST:
      strcpy(pktstateStr, "TH_RST");
      break;
   case TH_PUSH:
      strcpy(pktstateStr, "TH_PUSH");
      break;
   case TH_ACK:
      strcpy(pktstateStr, "TH_ACK");
      break;
   case TH_URG:
      strcpy(pktstateStr, "TH_URG");
      break;
   case TH_ECE:
      strcpy(pktstateStr, "TH_ECE");
      break;
   case TH_CWR:
      strcpy(pktstateStr, "TH_CWR");
      break;
   }
   return pktstateStr;
}
void
bsd_tcp_input(m, off0)
	...
	case TCPS_TIME_WAIT:
		...
		if (SEQ_LEQ(th->th_ack, tp->snd_una)) {
			if (tlen == 0 && tiwin == tp->snd_wnd) {
				 ...
				 * network.
				 */
				if (//!callout_active(tp->tt_rexmt) ||LUOYU
