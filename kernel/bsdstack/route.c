...
route_init(void)
{
	rtzone = uma_zcreate("rtentry", sizeof(struct rtentry), 20, NULL, NULL,
	    UMA_ALIGN_PTR, 0);
...
#ifdef INET
	/* Multicast goop, grrr... */
	//return mrt_ioctl ? mrt_ioctl(req, data) : EOPNOTSUPP;LUOYU
#else /* INET */
	return ENXIO;
#endif /* INET */
}
...
rt_maskedcopy(struct sockaddr *src, struct sockaddr *dst, struct sockaddr *netmask)
{
	...
	*cp2++ = *cp1++; *cp2++ = *cp1++; /* copies sa_len & sa_family */
	...
	while (cp2 < cplim)
		*cp2++ = *cp1++ & *cp3++;
	if (cp2 < cplim2)   //LUOYU delete!因为这里会改写地址，所以去掉，不过要研究研究
...
#define senderr(x) { error = x ; goto bad; }
	...
	makeroute:
		rt = uma_alloc(rtzone, M_NOWAIT | M_ZERO);
		if (rt == NULL)
			senderr(ENOBUFS);
		RT_LOCK_INIT(rt);
...
bad:
	RADIX_NODE_HEAD_UNLOCK(rnh);
/*
 * KJZ added the following 2 lines to support add ether addr with same submask
 */ 
	if (error == EEXIST)
		error = 0;
