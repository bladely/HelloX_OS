...
ip_init()
{
	...
	in_ifaddrhashtbl = hashinit(INADDR_NHASH, NULL, &in_ifaddrhmask);
	...
	netisr_register(NETISR_IP, bsd_ip_input, &ipintrq, NETISR_MPSAFE);
}
...
bsd_ip_input(struct mbuf *m)
{
	...
	int s, error;
#endif /* FAST_IPSEC */
	int ip_offset = 0; //LUOYU added for ip_off change
  	M_ASSERTPKTHDR(m);
  	//printf("ip_input\n");
...
passin:
	...
	if (ipforwarding == 0) {
		...
		if (sp == NULL) {	/* NB: can happen if error */
			splx(s);
			/*XXX error stat???*/
			DPRINTF(("bsd_ip_input: no SP for forwarding\n"));	/*XXX*/
			goto bad;
		}
...
ours:
	 ...
	 */
	ip_offset = htons(ip->ip_off);//ip->ip_off is 0x4000 when recved, and will be turn to 0x0040 before here  
	...
	if ((inetsw[ip_protox[ip->ip_p]].pr_flags & PR_LASTHDR) != 0) {
		...
		if (sp != NULL) {
			...
			/* XXX error stat??? */
			error = EINVAL;
DPRINTF(("bsd_ip_input: no SP, packet discarded\n"));/*XXX*/
			goto bad;
		}
		splx(s);
		if (error)
...
#define	SA	struct sockaddr *
		...
		case IPOPT_TS:
			...
			if (off > optlen - (int)sizeof(__int32_t)) {
			...
			ntime = iptime();
			(void)memcpy(cp + off, &ntime, sizeof(__uint32_t));
			cp[IPOPT_OFFSET] += sizeof(__uint32_t);
		}
	}
...
#define	RTA(rt)	((struct in_ifaddr *)(rt->rt_ifa))
	...
	error = bsd_ip_output(m, (struct mbuf *)0, NULL, IP_FORWARDING, 0, NULL);
	if (error)
		ipstat.ips_cantforward++;
	else {
		ipstat.ips_forward++;
